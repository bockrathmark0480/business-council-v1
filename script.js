...
async function fetchDeliberationResults({ roles, question, context }) {
  const response = await fetch("/api/deliberate", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ roles, question, context }),
  });

  if (!response.ok) {
    const message = await response.text();
    throw new Error(message || "Failed to reach the Core Brain/EA service");
  }
...
async function runDeliberationFlow({
  ...
  const addInsight = (role, output, emphasis) => {
    ...
    paragraph.textContent = output?.summary || "No insight returned.";
    body.appendChild(paragraph);

    if (output?.stance) {
      const stanceLine = document.createElement("p");
      stanceLine.className = "meta";
      stanceLine.textContent = `Stance: ${output.stance}`;
      body.appendChild(stanceLine);
    }

    if (output?.riskFlags?.length) {
      const risks = document.createElement("ul");
      risks.className = "risk-flags";
      output.riskFlags.forEach((flag) => {
        const li = document.createElement("li");
        li.textContent = flag;
  ...
  let deliberationPayload;
  try {
    deliberationPayload = await fetchDeliberationResults({ roles, question, context });
  } catch (error) {
    addTimelineItem(
      "Core Brain/EA",
      "We could not reach the council intelligence service. Please try again.",
      { type: "error" }
    );
    summaryBody.textContent =
      "The Core Brain/EA could not complete the deliberation due to a connectivity issue.";
    completed = steps;
    updateProgress();
    throw error;
  }

  const roleOutputs = deliberationPayload?.roles || [];
  const coreSynthesis = deliberationPayload?.coreSynthesis || {};
  const roleMap = new Map(roleOutputs.map((entry) => [entry.roleId, entry]));

  for (const role of roles) {
    const output = roleMap.get(role.id) || null;
    const emphasis = output?.weight ?? Math.round((role.reasoning / baseWeight) * 100);
    const stanceNote = output?.stance ? ` Stance: ${output.stance}.` : "";
    const risksNote = output?.riskFlags?.length
      ? ` Risks: ${output.riskFlags.join("; ")}.`
      : "";
    const reasoning = output?.summary
      ? `${output.summary}${stanceNote}${risksNote}`.trim()
      : `Awaiting insight from ${role.name}.`;
    ...
    addInsight(role, output, emphasis);
  ...
  await delay(350);
  const summaryText = coreSynthesis.summary ||
    "The Core Brain/EA could not synthesize a final answer. Please retry the deliberation.";
  summaryBody.textContent = summaryText;

  addTimelineItem(
    "Core Brain/EA Return",
    coreSynthesis.returnNote ||
      "Synthesizes the weighted debate into a unified directive for the user.",
    {
      type: "summary",
  ...
  const actions = coreSynthesis.actionPlan?.length
    ? coreSynthesis.actionPlan
    : generateActionPlan(roles, question, context, baseWeight);
